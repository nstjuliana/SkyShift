...
exports.getWeatherForFlight = getWeatherForFlight;
exports.clearWeatherCache = clearWeatherCache;
const weather_tomorrow_1 = require("./weather-tomorrow");
const weather_openweather_1 = require("./weather-openweather");
/**
...
async function getWeatherForFlight(location, scheduledDate) {
    ...
    if (useTomorrowIO) {
        ...
        data = await weather_tomorrow_1.tomorrowClient.getForecast(location.latitude, location.longitude, scheduledDate.toISOString());
    }
    else {
        // Use OpenWeather for flights >= 4 days away
        console.log('[Weather Router] Calling OpenWeather API...');
        data = await weather_openweather_1.openWeatherClient.getForecast(location.latitude, location.longitude, scheduledDate);
    }
    // Cache the result
