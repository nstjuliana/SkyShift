...
import { openai, AI_MODEL, AI_TEMPERATURE } from '@/lib/ai';
import { getWeatherForFlight } from '@/lib/weather';
import { WEATHER_MINIMUMS } from '@/constants/weather-minimums';
import { aiRescheduleResponseSchema, type RescheduleOption } from '@/types/reschedule';
...
class AIRescheduleService {
  ...
  async generateRescheduleOptions(bookingId: string): Promise<RescheduleOption[]> {
    ...
    const weatherForecasts = await Promise.all(
      potentialDates.slice(0, 5).map(async (date) => {
        try {
          const weather = await getWeatherForFlight(departureLocation, date);
          return {
    ...
    const validForecasts = weatherForecasts.filter((f): f is NonNullable<typeof f> => f !== null);
    ...
    const parsed = aiRescheduleResponseSchema.parse(aiResponse);

    // Validate dates and check instructor availability
    const validatedOptions = await this.validateAndFilterOptions(
      parsed.options,
  ...
  private buildPrompt(
    booking: any,
    weatherForecasts: Array<{ date: string; weather: any }>,
    minimums: any,
...
- Location: ${(booking.departureLocation as Location).name} (${(booking.departureLocation as Location).latitude}, ${(booking.departureLocation as Location).longitude})
...
Return ONLY valid JSON, no markdown formatting.`;
   ...
   * @private
   */
  private async validateAndFilterOptions(
    options: RescheduleOption[],
    instructorId: string,
    duration: number
  ): Promise<RescheduleOption[]> {
    const now = new Date();
    const validated: RescheduleOption[] = [];
  ...
  private async checkInstructorAvailability(
    ...
    const conflictingFlight = await db.booking.findFirst({
      where: {
        instructorId,
        scheduledDate: {
          gte: startTime,
...
export const aiRescheduleService = new AIRescheduleService();
