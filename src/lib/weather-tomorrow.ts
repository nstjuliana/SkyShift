...
interface TomorrowTimelineItem {
  ...
  values: {
    temperature: number; // Fahrenheit (with units=imperial)
    windSpeed: number; // m/s (or mph with units=imperial, but we convert to knots)
    windDirection: number; // degrees
    windGust: number; // m/s (or mph with units=imperial, but we convert to knots)
    visibility: number; // miles (with units=imperial) or meters (with units=metric)
    cloudCover: number; // percentage
    cloudBase?: number; // Cloud base height in miles (with units=imperial) or meters (with units=metric)
    precipitationIntensity: number; // mm/h
    precipitationType: number; // 0=none, 1=rain, 2=snow, etc.
    weatherCode: number;
  };
}
...
interface TomorrowResponse {
  timelines: Array<{
    timestep: string;
    intervals: TomorrowTimelineItem[];
  }>;
}
...
class TomorrowClient {
  ...
  private readonly baseUrl = 'https://api.tomorrow.io/v4/timelines';

  constructor() {
    if (!env.TOMORROW_IO_API_KEY) {
      throw new Error('TOMORROW_IO_API_KEY is not configured');
    }
    this.apiKey = env.TOMORROW_IO_API_KEY;
  }
   ...
   */
  async getForecast(lat: number, lon: number, timestampISO: string): Promise<WeatherData> {
    try {
   ...
   * @private
   */
  private transformResponse(item: TomorrowTimelineItem): WeatherData {
...
export const tomorrowClient = new TomorrowClient();
