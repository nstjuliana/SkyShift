...
class FlightService {
   ...
   */
  async getFlightById(id: string, userId: string, userRole: UserRole) {
    try {
      const flight = await db.booking.findUnique({
        where: { id },
        include: {
          student: {
            select: {
   ...
   */
  async updateFlightStatus(data: UpdateFlightInput) {
    try {
      const flight = await db.booking.update({
        where: { id: data.id },
        data: {
          status: data.status,
